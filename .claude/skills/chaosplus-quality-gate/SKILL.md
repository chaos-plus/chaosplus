---
name: chaosplus-quality-gate
description: Route Chaosplus repository work to the correct domain skill and enforce repository-wide quality gates. Use for any implementation, refactor, review, test, release, architecture, API, frontend, documentation, deployment, or skill change in this repository, especially when changed paths span multiple domains or acceptance readiness must be assessed.
---

# Chaosplus Quality Gate
...
```text
python .agents/skills/chaosplus-quality-gate/scripts/skill-runtime.py refresh
```
...
| --- | --- |
| `cmd/**`, `internal/**`, `pkg/**`, `go.mod`, `go.sum`, backend deployment | `$chaosplus-backend` |
| `web/admin/**`, frontend container or proxy | `$chaosplus-frontend` |
| `web/docs/**`, `docs/**`, `README.md` | `$chaosplus-docs` |
| `.agents/**`, `AGENTS.md`, `.github/**`, cross-domain release | `$chaosplus-quality-gate` plus every affected domain |
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .agents/skills/chaosplus-quality-gate/scripts/check-gates.ps1
```

Run all release gates before an acceptance or open-source-ready claim:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .agents/skills/chaosplus-quality-gate/scripts/check-gates.ps1 -Scope all -Full
```
