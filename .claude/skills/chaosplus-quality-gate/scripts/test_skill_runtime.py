...
class SkillRuntimeTest(unittest.TestCase):
    ...
    def create_skill(self, name: str = "chaosplus-backend") -> Path:
        ...
        (skill / "agents" / "openai.yaml").write_text(
            "interface:\n"
            '  display_name: "Chaosplus Backend"\n'
    ...
    def create_repository(self) -> None:
        ...
        for path in (
            "internal/modules/iam/sql/sqlite",
            "internal/modules/iam/sql/mysql",
            "internal/modules/iam/sql/postgres",
            "internal/modules/oauth",
            "web/admin/apps/web",
            "web/docs",
        ):
            (self.root / path).mkdir(parents=True, exist_ok=True)
        (self.root / "internal" / "modules" / "iam" / "module.go").write_text("package iam\n", encoding="utf-8")
        (self.root / "web" / "admin" / "package.json").write_text(
            json.dumps({"name": "admin", "packageManager": "bun@1", "workspaces": ["apps/*"]}),
            encoding="utf-8",
        )
        (self.root / "web" / "admin" / "apps" / "web" / "package.json").write_text(
            json.dumps({"scripts": {"test": "bun test src"}}), encoding="utf-8"
        )
        (self.root / "web" / "docs" / "package.json").write_text(
            json.dumps(
                {
                    "name": "docs",
        ...
        quality = self.create_skill("chaosplus-quality-gate")
    ...
    def test_refresh_is_deterministic_and_idempotent(self) -> None:
        ...
        facts = self.skills / "chaosplus-quality-gate" / "references" / "repository-facts.md"
