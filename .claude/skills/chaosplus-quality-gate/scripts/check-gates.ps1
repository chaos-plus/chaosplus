...
function Resolve-Scopes {
    ...
    return @{
        backend = [bool]($paths | Where-Object { $_ -match '^(cmd|internal|pkg|deploy)/|^go\.(mod|sum)$|^Dockerfile$|^\.agents/skills/chaosplus-backend/' })
        frontend = [bool]($paths | Where-Object { $_ -match '^web/admin/|^\.agents/skills/chaosplus-frontend/' })
        docs = [bool]($paths | Where-Object { $_ -match '^(web/docs|docs)/|^README\.md$|^\.agents/skills/chaosplus-docs/' })
    }
}
...
Invoke-NativeStep 'Test repository skill runtime' $repoRoot 'python' @('-m', 'unittest', 'discover', '-s', '.agents/skills/chaosplus-quality-gate/scripts', '-p', 'test_*.py')
...
if ($selected.frontend) {
    ...
    foreach ($file in @('Dockerfile', 'nginx.conf')) {
        if (-not (Test-Path (Join-Path $adminRoot $file))) { $failures.Add("Frontend deployment file is missing: web/admin/$file") }
    }
