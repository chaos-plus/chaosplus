---
name: chaosplus-docs
description: Develop, reorganize, validate, and publish the Chaosplus documentation site and architecture documentation. Use for changes under web/docs or docs, README documentation, Astro/Starlight configuration, Mermaid diagrams, API and deployment guides, navigation, document synchronization, link checks, or documentation containers.
---
...
4. Treat `web/docs` as the Astro/Starlight publication layer. Use its sync script for mirrored source pages; do not hand-edit generated copies.
...
- Remove checks that only validate copied SevenLink artifacts; retain or replace them with Chaosplus-relevant build, link, structure, and protected-build checks.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .agents/skills/chaosplus-quality-gate/scripts/check-gates.ps1 -Scope docs
```
...
```text
python .agents/skills/chaosplus-quality-gate/scripts/skill-runtime.py record --domain docs --symptom "..." --cause "..." --prevention "..." --evidence "build or link-check output"
```
