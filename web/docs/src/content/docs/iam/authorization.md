...
当前约束覆盖 tenant role、静态/动态组和岗位派生 role、治理型临时 role grant、角色权限条件、角色 owner/department 数据范围、entity scoped binding，以及 tenant 内实体图、终端业务资源 ReBAC 和关系条件。访问申请、四眼审批、撤销、到期控制及直接/临时租户角色授权复核已实现，详见 [访问治理设计](/iam/access-governance/)；资源属性 ABAC 和派生授权复核仍未完成。
