...
function MemberDrawer({ member, roles, onClose, onChanged }: { member: Member | null; roles: Role[]; onClose: () => void; onChanged: () => Promise<void> }) {
	...
	const [busy, setBusy] = useState(false)
	useEffect(() => { if (member) { setRolesLoading(true); api.memberRoles(member.subject).then((data) => setAssigned(data ?? [])).catch(() => setAssigned([])).finally(() => setRolesLoading(false)) } }, [member])
  if (!member) return null
	const toggleRole = async (role: Role, checked: boolean) => { setBusy(true); try { if (checked) await api.addRoleMember(role.id, member.subject); else await api.removeRoleMember(role.id, member.subject); setAssigned((await api.memberRoles(member.subject)) ?? []) } finally { setBusy(false) } }
  const toggleStatus = async () => { setBusy(true); try { await api.updateMember(member.subject, { status: member.status === 'active' ? 'disabled' : 'active' }); await onChanged() } finally { setBusy(false) } }
	return <div className="drawer-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><aside className="detail-drawer"><header><div><h2>{member.display_name}</h2><code>{member.subject}</code></div><button className="button secondary" onClick={onClose}>关闭</button></header><section><h3>成员状态</h3><div className="status-line"><Badge status={member.status} /><button className={`button ${member.status === 'active' ? 'danger' : 'primary'}`} disabled={busy} onClick={toggleStatus}>{member.status === 'active' ? '停用成员' : '重新启用'}</button></div></section><section><h3>角色分配</h3><div className="check-list">{roles.map((role) => <label key={role.id}><input type="checkbox" checked={assigned.includes(role.id)} disabled={rolesLoading || busy || member.status !== 'active'} onChange={(e) => toggleRole(role, e.target.checked)} /><span><strong>{role.name}</strong><small>{role.description || role.id}</small></span></label>)}</div></section></aside></div>
}
