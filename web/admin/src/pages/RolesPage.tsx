...
export default function RolesPage() {
  ...
  const toggle = async (code: string, checked: boolean) => { if (!selected) return; setBusy(true); try { if (checked) await api.grantPermission(selected.id, code); else await api.revokePermission(selected.id, code); setGranted(await api.rolePermissions(selected.id)) } finally { setBusy(false) } }
  const remove = async () => { if (!selected || !confirm(`删除角色“${selected.name}”？`)) return; await api.deleteRole(selected.id); setSelected(null); await refresh() }
