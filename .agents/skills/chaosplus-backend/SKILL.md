---
name: chaosplus-backend
description: Develop, review, test, and refactor the Chaosplus Go backend and IAM APIs. Use for changes under cmd, internal, pkg, Go modules, SQL migrations, OpenAPI contracts, authentication, authorization, tenancy, database support, runtime plugins, deployment bootstrap, or backend tests.
---
...
2. Read `../chaosplus-quality-gate/references/repository-facts.md`.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .agents/skills/chaosplus-quality-gate/scripts/check-gates.ps1 -Scope backend
```
...
```text
python .agents/skills/chaosplus-quality-gate/scripts/skill-runtime.py record --domain backend --symptom "..." --cause "..." --prevention "..." --evidence "test or gate output"
```
