...
def render_repository_facts(repo_root: Path) -> str:
    go_mod = (repo_root / "go.mod").read_text(encoding="utf-8-sig")
    ...
    module_root = repo_root / "internal" / "modules"
    ...
    ignored_parts = {".git", ".local", "vendor", "web"}
    go_packages = {
        path.parent
    ...
    admin = _read_json(repo_root / "web" / "admin" / "package.json")
    admin_app = _read_json(repo_root / "web" / "admin" / "apps" / "web" / "package.json")
    docs = _read_json(repo_root / "web" / "docs" / "package.json")
    workspaces = ", ".join(str(value) for value in admin["workspaces"])

    return f"""# Chaosplus Repository Facts
...
def refresh_context(repo_root: Path) -> bool:
    facts_path = repo_root / ".agents" / "skills" / "chaosplus-quality-gate" / "references" / "repository-facts.md"
...
def record_learning(
    ...
    skill_name = "chaosplus-quality-gate" if domain == "quality-gate" else f"chaosplus-{domain}"
...
def main(arguments: list[str]) -> int:
    ...
    if args.command == "validate":
        ...
        errors = validate_skills(repo_root / ".agents" / "skills", selected)
        if errors:
            print("Skill validation failed:", file=sys.stderr)
            for error in errors:
                print(f"- {error}", file=sys.stderr)
            return 1
        count = len(selected) if selected is not None else len(list((repo_root / ".agents" / "skills").iterdir()))
