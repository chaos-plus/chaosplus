...
企业联邦、派生授权复核和 WORM root anchoring 尚未实现。SCIM 2.0 入站预配已实现，协议、事务和运维契约见 [SCIM 2.0 预配](scim-provisioning.md)。访问申请、四眼审批、租户角色临时授权和直接/临时授权复核已实现，详细事务与授权契约见 [访问治理设计](access-governance.md)。自助注册、邮箱验证与密码找回页面和后端已实现，但部署默认关闭；只有真实通知供应商就绪并通过投递验收后才可启用。
