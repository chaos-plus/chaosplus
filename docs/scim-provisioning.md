...
Chaosplus 作为 SCIM Service Provider 接收企业身份源的用户和组预配。每个 SCIM directory 固定属于一个 tenant；Bearer 凭据、资源映射、查询和写入都不能越过该 tenant。当前实现覆盖 Users、Groups、PATCH、Bulk、过滤、分页、发现资源、弱 ETag 和停用预配，另提供出站 SCIM client（见 [出站 SCIM 目标](deployment.md#出站-scim-目标可选)），不包含定时拉取或企业 OIDC/SAML federation。
...
- `web/admin/apps/web/src/lib/iam-api.test.ts`：真实 TCP typed client；
- `web/admin/apps/web/scripts/scim-browser-audit.mjs`：真实后端与 Chromium 管理/协议工作流。
