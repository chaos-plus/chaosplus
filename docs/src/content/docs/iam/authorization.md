---
title: "授权设计"
description: "由 docs/authz-design.md 同步的工程文档"
editUrl: false
---
...
> 总体目标与尚未实现能力见 [IAM 平台架构](/architecture/iam-platform/)。
...
当前约束覆盖 tenant role、静态/动态组和岗位派生 role、治理型临时 role grant、角色权限条件、角色 owner/department 数据范围、entity scoped binding，以及 tenant 内实体图、终端业务资源 ReBAC 和关系条件。访问申请、四眼审批、撤销、到期控制及直接/临时租户角色授权复核已实现，详见 [访问治理设计](/iam/access-governance/)；派生授权复核与资源属性 ABAC 已实现。
...
完整写入契约见 [IAM 授权写入](/iam/authorization-writes/)。
