# Machine Runner 接入 + daemon WS 链路 + CLI 自动检测 设计
...
把 daemon(执行面)↔ 控制面的传输从 **NATS 直连** 改为 **WS 长连 + token 认证**,NATS 彻底内部化;实现 PRD §5.3.1 的 **Machine 接入流程**(5 分钟一次性 token → 确认转长期 → 心跳/重连);daemon 的 claude/codex 运行时 **自动检测**。外部只暴露控制面一个 HTTP/WS 端口。

## 2. 传输(对齐 J3/C3)

```
daemon:  bun run src/serve.ts --server <url> --token <token> [--name <name>]
控制面:  WS daemon hub — 握手带 token 认证 → runnerID↔conn 映射
         → 下发 spawn/kill/read-file/run-cmd → 回收事件 + 15s 心跳
NATS:    仅控制面内部引擎事件 fan-out(chaos.run.*);daemon 链路 NATS gateway 移除
CLI 自动检测:claude/codex 走 PATH 探测 + 本地配置(cc-switch / ~/.codex);env 仅 override
```

- daemon 不再知道 NATS;`--server` 缺省 `http://127.0.0.1:8081`(本地 dev)。
- 命令协议沿用现有 daemon 命令集(spawn/kill/switch-provider/read-file/run-cmd),载体从 NATS 改 WS。
...
daemon --token 连 WS → machine=pending,UI 倒计时
...
### 4.2 控制面(Go)
- **WS daemon hub**(`internal/machine/`):接受 daemon WS 连接,握手校验 token;`runnerID↔conn` 映射;路由 spawn/kill/read-file/run-cmd;回收事件 + 心跳;断线标记离线。
...
- **RunnerExecutor 换 WS-backed**:现有 `RunnerExecutor` 依赖 `*gateway.Gateway`(NATS)。抽象 `RunnerLink` 接口(SpawnAndWait/ReadArtifact/RunCmd/Kill/RegisteredRunners),RunnerExecutor 走接口;本特性实现 WS-backed(经 hub),NATS 实现保留但 daemon 链路默认 WS。
...
GET  /api/machines/ws?token=...     → WS upgrade(daemon 接入,握手即注册)
...
```
- daemon 事件走同一 WS 连接回传(不另开通道)。

## 7. 错误处理 / 测试 / 验收
- token 无效/过期 → WS 握手 401 拒绝;确认未连接不可点;重复确认幂等。
- 测试:daemon WS client↔hub 握手、spawn 往返、心跳;token 签发/校验/过期/长期化;machine store CRUD + 状态机;RunnerExecutor WS 路径(mock)。
...
- NATS daemon 链路代码保留(不删),默认切换到 WS。
