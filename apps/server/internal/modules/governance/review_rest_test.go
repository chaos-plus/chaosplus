...
func TestAccessReviewHTTPWorkflowAndOpenAPI(t *testing.T) {
	...
	createdResponse := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", map[string]any{
	...
	listed := governanceRequest(t, server, http.MethodGet, "/iam/access-reviews", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", nil)
	...
	loaded := governanceRequest(t, server, http.MethodGet, "/iam/access-reviews/"+review.ID.String(), wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", nil)
	assert.Equal(t, http.StatusOK, loaded.status, loaded.Message)
	decided := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews/"+review.ID.String()+"/items/"+review.Items[0].ID.String()+"/decide", wireID("tenant-a"), "other", authn.SubjectTypePrincipal, "en-US", map[string]any{
		"decision": ReviewDecisionKeep, "note": "still required",
	})
	assert.Equal(t, http.StatusOK, decided.status, decided.Message)
	completed := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews/"+review.ID.String()+"/complete", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", nil)
	assert.Equal(t, http.StatusOK, completed.status, completed.Message)

	addDirectReviewGrant(t, fixture, "other", governanceNow)
	cancelResponse := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", map[string]any{
	...
	cancelled := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews/"+cancelledReview.ID.String()+"/cancel", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", nil)
...
func TestAccessReviewHTTPErrorsAreLocalized(t *testing.T) {
	...
	for _, item := range []struct {
		...
		response := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, item.locale, map[string]any{
	...
	serviceAccount := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews", wireID("tenant-a"), "service", authn.SubjectTypeServiceAccount, "ms-MY", map[string]any{
	...
	missing := governanceRequest(t, server, http.MethodGet, "/iam/access-reviews/"+wireID("missing"), wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", nil)
	...
	missingDecision := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews/"+wireID("missing")+"/items/"+wireID("missing")+"/decide", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", map[string]any{
		"decision": ReviewDecisionKeep,
	})
	assert.Equal(t, http.StatusNotFound, missingDecision.status)

	missingCompletion := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews/"+wireID("missing")+"/complete", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", nil)
	assert.Equal(t, http.StatusNotFound, missingCompletion.status)

	serviceDecision := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews/"+wireID("missing")+"/items/"+wireID("missing")+"/decide", wireID("tenant-a"), "service", authn.SubjectTypeServiceAccount, "en-US", map[string]any{
		"decision": ReviewDecisionKeep,
	})
	assert.Equal(t, http.StatusForbidden, serviceDecision.status)
	serviceCompletion := governanceRequest(t, server, http.MethodPost, "/iam/access-reviews/"+wireID("missing")+"/complete", wireID("tenant-a"), "service", authn.SubjectTypeServiceAccount, "en-US", nil)
	...
	listFailure := governanceRequest(t, brokenServer, http.MethodGet, "/iam/access-reviews", wireID("tenant-a"), "approver", authn.SubjectTypePrincipal, "en-US", nil)
