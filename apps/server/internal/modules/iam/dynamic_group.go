...
func matchingDynamicGroupIDs(ctx context.Context, db bun.IDB, tenantID, subject guid.ID) ([]guid.ID, error) {
	rows := make([]dynamicGroupRuleRow, 0)
	if err := db.NewSelect().TableExpr("iam_groups AS g").
		ColumnExpr("g.id, g.rule_json, tm.principal_id AS subject, tm.email, COALESCE(md.department_id, 0) AS department_id, tm.status").
		Join("JOIN iam_tenant_members AS tm ON tm.tenant_id = g.tenant_id AND tm.principal_id = ? AND tm.status = ?", subject, MemberActive).
