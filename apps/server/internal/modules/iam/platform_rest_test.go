...
func seedRESTPrincipal(t *testing.T, db *bun.DB, principalID guid.ID) {
	t.Helper()
...
func TestPlatformAdministratorHTTPLifecycle(t *testing.T) {
	...
	seedRESTPrincipal(t, db, wireGUID("operator-2"))
	...
	assert.Equal(t, authorization.principalID.String(), administrators[0].PrincipalID)
	...
	restricted := api.Put("/iam/platform-administrators/"+wireID("operator-2")+"", header, map[string]any{
	...
	promoted := api.Put("/iam/platform-administrators/"+wireID("operator-2")+"", header, map[string]any{"full_administrator": true})
	...
	removed := api.Delete("/iam/platform-administrators/"+wireID("operator-2")+"", header)
	require.Equal(t, http.StatusOK, removed.Code, removed.Body.String())
	assert.Equal(t, http.StatusNotFound, api.Delete("/iam/platform-administrators/"+wireID("operator-2")+"", header).Code)
}

func TestPlatformAdministratorHTTPFailures(t *testing.T) {
	db, api, authorization := newIAMAPI(t)
	header := authorization.header
	seedRESTPrincipal(t, db, wireGUID("operator-3"))

	// Tenant-scoped and unknown codes are refused.
	scope := api.Put("/iam/platform-administrators/"+wireID("operator-3")+"", header, map[string]any{
	...
	unknown := api.Put("/iam/platform-administrators/"+wireID("operator-3")+"", header, map[string]any{
	...
	conflicting := api.Put("/iam/platform-administrators/"+wireID("operator-3")+"", header, map[string]any{
	...
	empty := api.Put("/iam/platform-administrators/"+wireID("operator-3")+"", header, map[string]any{"full_administrator": false})
	...
	last := api.Delete("/iam/platform-administrators/"+authorization.principalID.String(), header)
	assert.Equal(t, http.StatusConflict, last.Code, last.Body.String())
	demote := api.Put("/iam/platform-administrators/"+authorization.principalID.String(), header, map[string]any{
...
func TestPlatformRoutesRejectNonPlatformPrincipal(t *testing.T) {
	...
	for _, code := range []int{
		...
		api.Delete("/iam/platform-administrators/"+wireID("whoever"), authorization.header).Code,
	} {
