...
func TestRetentionPolicyDefaultAndUpsert(t *testing.T) {
	...
	policy, err := service.GetRetentionPolicy(t.Context(), testID("tenant-ret"))
	require.NoError(t, err)
	assert.Equal(t, testID("tenant-ret"), policy.TenantID)
	assert.Equal(t, defaultMinRetentionDays, policy.MinDays)
	assert.Equal(t, defaultArchiveAfterDays, policy.ArchiveAfterDays)

	policy, err = service.SetRetentionPolicy(t.Context(), testID("tenant-ret"), 90, 180)
	require.NoError(t, err)
	assert.Equal(t, 90, policy.MinDays)
	assert.Equal(t, 180, policy.ArchiveAfterDays)
	assert.False(t, policy.UpdatedAt.IsZero())

	again, err := service.GetRetentionPolicy(t.Context(), testID("tenant-ret"))
	require.NoError(t, err)
	assert.Equal(t, policy, again, "the upserted policy must round-trip")

	updated, err := service.SetRetentionPolicy(t.Context(), testID("tenant-ret"), 30, 365)
	require.NoError(t, err)
	assert.Equal(t, 30, updated.MinDays)
	assert.Equal(t, 365, updated.ArchiveAfterDays)

	other, err := service.GetRetentionPolicy(t.Context(), testID("tenant-other"))
	require.NoError(t, err)
...
func TestSetRetentionPolicyValidation(t *testing.T) {
	...
	_, err = service.SetRetentionPolicy(t.Context(), testID("tenant"), 0, 10)
	assert.ErrorIs(t, err, ErrRetentionInvalid)
	_, err = service.SetRetentionPolicy(t.Context(), testID("tenant"), 40000, 40001)
	assert.ErrorIs(t, err, ErrRetentionInvalid)
	_, err = service.SetRetentionPolicy(t.Context(), testID("tenant"), 365, 30)
	assert.ErrorIs(t, err, ErrRetentionInvalid, "an archive threshold before the minimum retention must be refused")
	_, err = service.GetRetentionPolicy(t.Context(), 0)
	assert.ErrorIs(t, err, ErrRetentionInvalid)
}
...
func TestGovernanceReportsRetentionAndStats(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, err = service.Append(t.Context(), EventInput{TenantID: testID("tenant-gov"), EventType: "created", Outcome: "success"})
		require.NoError(t, err)
	}
	_, err = service.SetRetentionPolicy(t.Context(), testID("tenant-gov"), 30, 90)
	require.NoError(t, err)

	report, err := service.Governance(t.Context(), testID("tenant-gov"))
	require.NoError(t, err)
...
func TestSignedRootRealWORMLifecycle(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, err = service.Append(t.Context(), EventInput{TenantID: testID("tenant-signed"), EventType: "created", Outcome: "success"})
		require.NoError(t, err)
	}
	root, err := service.SignRoot(t.Context(), testID("tenant-signed"))
	require.NoError(t, err)
	...
	integrity, err := service.Verify(t.Context(), testID("tenant-signed"))
	require.NoError(t, err)
	...
	again, err := service.SignRoot(t.Context(), testID("tenant-signed"))
	require.NoError(t, err)
	assert.Equal(t, root, again, "signing an already signed root must be idempotent")

	report, err := service.Governance(t.Context(), testID("tenant-signed"))
	require.NoError(t, err)
...
func TestSignedRootRejectsTamperedAnchor(t *testing.T) {
	...
	require.NoError(t, iam.Migrate(t.Context(), db))
	service := NewServiceWithAnchorAndSigner(db, store, signer, newTestIDGenerator())
	_, err = service.Append(t.Context(), EventInput{TenantID: testID("tenant-tamper"), EventType: "created", Outcome: "success"})
	require.NoError(t, err)
	root, err := service.SignRoot(t.Context(), testID("tenant-tamper"))
	require.NoError(t, err)
...
func TestSignRootWithoutSignerFailsClosed(t *testing.T) {
	...
	_, err = service.Append(t.Context(), EventInput{TenantID: testID("tenant-unsigned"), EventType: "created", Outcome: "success"})
	require.NoError(t, err)
	anchor, err := service.Anchor(t.Context(), testID("tenant-unsigned"))
	require.NoError(t, err)
	assert.Empty(t, anchor.RootSignature)

	_, err = service.SignRoot(t.Context(), testID("tenant-unsigned"))
	assert.ErrorIs(t, err, ErrRootSigningDisabled, "an anchor committed before signing was enabled must fail closed")

	integrity, err := service.Verify(t.Context(), testID("tenant-unsigned"))
	require.NoError(t, err)
	assert.True(t, integrity.Valid)
	assert.True(t, integrity.Anchor.Valid)
