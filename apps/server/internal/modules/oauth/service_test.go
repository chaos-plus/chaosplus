...
func TestAuthorizationCodePKCEAndRefreshRotation(t *testing.T) {
	...
	client := clientRow{ID: testID("web"), TenantID: testID("tenant"), Name: "Web", RedirectURIs: `["https://client.example/callback"]`, GrantTypes: "authorization_code,refresh_token", Scopes: "openid profile email", PublicClient: true, Status: "active", CreatedAt: now, UpdatedAt: now}
	...
	redirect, err := service.Authorize(context.Background(), auth.SessionCookie(session), guidString(client.ID), "https://client.example/callback", "code", "openid profile", "state", challenge, "S256", "nonce", "")
	require.NoError(t, err)
	parsed, err := url.Parse(redirect)
	require.NoError(t, err)
	code := parsed.Query().Get("code")
	response, err := service.Token(context.Background(), url.Values{"grant_type": {"authorization_code"}, "client_id": {guidString(client.ID)}, "code": {code}, "redirect_uri": {"https://client.example/callback"}, "code_verifier": {verifier}}, "")
	require.NoError(t, err)
	...
	assert.Equal(t, guidString(client.ID), claims.ClientID)
	assert.Equal(t, 1, claims.ACR)
	assert.Equal(t, []string{"pwd"}, claims.AMR)
	assert.False(t, claims.AuthTime.IsZero())
	rotated, err := service.Token(context.Background(), url.Values{"grant_type": {"refresh_token"}, "client_id": {guidString(client.ID)}, "refresh_token": {response.RefreshToken}}, "")
	require.NoError(t, err)
	...
	_, err = db.ExecContext(context.Background(), "UPDATE iam_tenant_members SET status = 'disabled' WHERE tenant_id = ? AND principal_id = ?", testID("tenant"), parseGUID(principalID))
	require.NoError(t, err)
	...
	_, err = db.ExecContext(context.Background(), "UPDATE iam_tenant_members SET status = 'active' WHERE tenant_id = ? AND principal_id = ?", testID("tenant"), parseGUID(principalID))
	require.NoError(t, err)
...
func TestClientManagementTenantIsolation(t *testing.T) {
	...
	client, secret, err := service.CreateClient(context.Background(), testID("tenant-a"), "Worker", nil, []string{"client_credentials"}, []string{"jobs.read"}, false)
	require.NoError(t, err)
	assert.NotEmpty(t, secret)

	other, err := service.ListClients(context.Background(), testID("tenant-b"))
	require.NoError(t, err)
	assert.Empty(t, other)
	_, err = service.UpdateClient(context.Background(), testID("tenant-b"), parseGUID(client.ID), "Changed", nil, []string{"client_credentials"}, []string{"jobs.read"}, false, "active")
	assert.ErrorIs(t, err, ErrInvalidRequest)
	_, err = service.RotateClientSecret(context.Background(), testID("tenant-b"), parseGUID(client.ID))
	assert.ErrorIs(t, err, ErrInvalidRequest)
	assert.ErrorIs(t, service.DeleteClient(context.Background(), testID("tenant-b"), parseGUID(client.ID)), ErrInvalidRequest)

	owned, err := service.ListClients(context.Background(), testID("tenant-a"))
	require.NoError(t, err)
	require.Len(t, owned, 1)
	assert.Equal(t, client.ID, owned[0].ID)
	events, total, err := service.audit.List(t.Context(), auditmod.Filter{TenantID: testID("tenant-a"), Limit: 50})
	require.NoError(t, err)
...
func TestIntrospectionRequiresConfidentialClient(t *testing.T) {
	...
	public, _, err := service.CreateClient(context.Background(), testID("tenant"), "Browser", []string{"https://app.example/callback"}, []string{"authorization_code"}, []string{"openid"}, true)
	require.NoError(t, err)
	confidential, secret, err := service.CreateClient(context.Background(), testID("tenant"), "Resource Server", nil, []string{"client_credentials"}, []string{"introspect"}, false)
	require.NoError(t, err)
...
func TestClientCredentialsAndSecretRotation(t *testing.T) {
	...
	client, secret, err := service.CreateClient(t.Context(), testID("tenant"), "Worker", nil, []string{"client_credentials"}, []string{"jobs.read", "jobs.write"}, false)
	require.NoError(t, err)
	...
	updated, err := service.UpdateClient(t.Context(), testID("tenant"), parseGUID(client.ID), "Worker Updated", nil, []string{"client_credentials"}, []string{"jobs.read"}, false, "active")
	require.NoError(t, err)
	assert.Equal(t, "Worker Updated", updated.Name)
	rotated, err := service.RotateClientSecret(t.Context(), testID("tenant"), parseGUID(client.ID))
	require.NoError(t, err)
	...
	assert.ErrorIs(t, err, ErrInvalidRequest)
	assert.NoError(t, service.DeleteClient(t.Context(), testID("tenant"), parseGUID(client.ID)))
	_, err = authentication.Authenticate(t.Context(), "Bearer "+tokens.AccessToken, "")
	assert.ErrorIs(t, err, authnext.ErrInvalidToken)
}
...
func TestServiceAccountClientCredentials(t *testing.T) {
	service, authentication, _ := newOAuthTestService(t)
	identities := identity.NewService(service.db, oauthAuditAppender(service.audit), iam.NewAdministratorGuard(), oauthTestNextID)
	account, err := identities.CreateServiceAccount(t.Context(), testID("tenant"), "automation", "Automation", "", nil)
	require.NoError(t, err)
	credential, err := identities.CreateServiceAccountCredential(t.Context(), testID("tenant"), account.ID, "primary", []string{"jobs.read", "jobs.write"}, nil)
	require.NoError(t, err)

	form := url.Values{"grant_type": {"client_credentials"}, "client_id": {guidString(credential.Credential.ID)}, "client_secret": {credential.Secret}, "scope": {"jobs.read"}}
	...
	_, err = service.Token(t.Context(), form, "")
	assert.ErrorIs(t, err, ErrInvalidRequest)
	require.NoError(t, identities.RevokeServiceAccountCredential(t.Context(), testID("tenant"), account.ID, credential.Credential.ID))
...
func TestOAuthClientValidation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assert.ErrorIs(t, validateClient(parseGUID(test.tenant), test.client, test.redirects, test.grants, test.scopes, test.public), ErrInvalidRequest)
		})
	}
	assert.NoError(t, validateClient(testID("tenant"), "client", validRedirect, []string{"authorization_code", "refresh_token"}, validScopes, true))
...
func TestOAuthProtocolRejectionPaths(t *testing.T) {
	...
	public, _, err := service.CreateClient(t.Context(), testID("tenant"), "Browser", []string{"https://app.example/callback"}, []string{"authorization_code", "refresh_token"}, []string{"openid"}, true)
	require.NoError(t, err)
	confidential, secret, err := service.CreateClient(t.Context(), testID("tenant"), "Worker", nil, []string{"client_credentials"}, []string{"jobs.read"}, false)
	require.NoError(t, err)

	_, err = service.ListClients(t.Context(), 0)
	assert.ErrorIs(t, err, ErrInvalidRequest)
	_, err = service.UpdateClient(t.Context(), testID("tenant"), parseGUID(public.ID), "Browser", public.RedirectURIs, public.GrantTypes, public.Scopes, true, "unknown")
	assert.ErrorIs(t, err, ErrInvalidRequest)
	...
	require.NoError(t, err)
	_, err = service.db.ExecContext(t.Context(), "UPDATE iam_tenant_members SET status = 'disabled' WHERE tenant_id = ? AND principal_id = ?", testID("tenant"), parseGUID(principalID))
	require.NoError(t, err)
	...
	_, _, err = service.CreateClient(t.Context(), 0, "Worker", nil, []string{"client_credentials"}, []string{"jobs.read"}, false)
	assert.ErrorIs(t, err, ErrInvalidRequest)
	_, err = service.UpdateClient(t.Context(), 0, parseGUID(public.ID), "Browser", public.RedirectURIs, public.GrantTypes, public.Scopes, true, "active")
	assert.ErrorIs(t, err, ErrInvalidRequest)
...
func TestOAuthStoredGrantExpirationAndReplay(t *testing.T) {
	service, authentication, principalID := newOAuthTestService(t)
	client, _, err := service.CreateClient(t.Context(), testID("tenant"), "Browser", []string{"https://app.example/callback"}, []string{"authorization_code", "refresh_token"}, []string{"openid", "profile"}, true)
	require.NoError(t, err)
	session, _, err := authentication.Login(t.Context(), "admin", "correct horse battery staple", "")
	require.NoError(t, err)
	verifier := "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-._~"
	...
	_, err = service.db.NewInsert().Model(&refreshRow{IDHash: hashToken("expired-refresh"), FamilyID: "family", PrincipalID: parseGUID(principalID), ClientID: parseGUID(client.ID), Scope: "openid", CreatedAt: now - 1000, ExpiresAt: now - 1}).Exec(t.Context())
	require.NoError(t, err)
	...
	disabled, err := service.UpdateClient(t.Context(), testID("tenant"), parseGUID(client.ID), client.Name, client.RedirectURIs, client.GrantTypes, client.Scopes, true, "disabled")
	require.NoError(t, err)
...
func TestOAuthDatabaseFailurePropagation(t *testing.T) {
	...
	_, _, err := service.CreateClient(t.Context(), testID("tenant"), "Worker", nil, []string{"client_credentials"}, []string{"jobs.read"}, false)
	assert.ErrorContains(t, err, "create OAuth client")
	_, err = service.ListClients(t.Context(), testID("tenant"))
	assert.Error(t, err)
	_, err = service.UpdateClient(t.Context(), testID("tenant"), testID("client"), "Worker", nil, []string{"client_credentials"}, []string{"jobs.read"}, false, "active")
	assert.Error(t, err)
	_, err = service.RotateClientSecret(t.Context(), testID("tenant"), testID("client"))
	assert.Error(t, err)
	assert.Error(t, service.DeleteClient(t.Context(), testID("tenant"), testID("client")))
}
...
func TestOAuthAuthorizationPersistenceFailures(t *testing.T) {
	t.Run("authorization code storage", func(t *testing.T) {
		service, authentication, _ := newOAuthTestService(t)
		client, _, err := service.CreateClient(t.Context(), testID("tenant"), "Browser", []string{"https://app.example/callback"}, []string{"authorization_code"}, []string{"openid"}, true)
		require.NoError(t, err)
		session, _, err := authentication.Login(t.Context(), "admin", "correct horse battery staple", "")
		require.NoError(t, err)
	...
	t.Run("invalid stored redirect", func(t *testing.T) {
		service, authentication, _ := newOAuthTestService(t)
		now := time.Now().UTC().UnixMilli()
		row := clientRow{ID: testID("invalid-redirect"), TenantID: testID("tenant"), Name: "Browser", RedirectURIs: `["%"]`, GrantTypes: "authorization_code", Scopes: "openid", PublicClient: true, Status: "active", CreatedAt: now, UpdatedAt: now}
		_, err := service.db.NewInsert().Model(&row).Exec(t.Context())
		require.NoError(t, err)
		session, _, err := authentication.Login(t.Context(), "admin", "correct horse battery staple", "")
		require.NoError(t, err)
		_, err = service.Authorize(t.Context(), authentication.SessionCookie(session), guidString(row.ID), "%", "code", "openid", "", strings.Repeat("a", 43), "S256", "", "")
		assert.ErrorIs(t, err, ErrInvalidRequest)
	})

	t.Run("refresh token storage", func(t *testing.T) {
		service, _, principalID := newOAuthTestService(t)
		client, _, err := service.CreateClient(t.Context(), testID("tenant"), "Browser", []string{"https://app.example/callback"}, []string{"refresh_token"}, []string{"openid"}, true)
		require.NoError(t, err)
		now := service.now().UTC().UnixMilli()
		_, err = service.db.NewInsert().Model(&refreshRow{IDHash: hashToken("refresh"), FamilyID: "family", PrincipalID: parseGUID(principalID), ClientID: parseGUID(client.ID), Scope: "openid", CreatedAt: now, ExpiresAt: now + time.Hour.Milliseconds()}).Exec(t.Context())
		require.NoError(t, err)
...
func TestOAuthServicePropagatesDatabaseFailures(t *testing.T) {
	...
	_, listErr := service.ListClients(t.Context(), testID("tenant"))
	assert.Error(t, listErr)
	_, _, createErr := service.CreateClient(t.Context(), testID("tenant"), "Broken", []string{"https://app.example/cb"}, []string{"authorization_code"}, []string{"openid"}, true)
	assert.Error(t, createErr)
	_, updateErr := service.UpdateClient(t.Context(), testID("tenant"), testID("missing"), "Broken", []string{"https://app.example/cb"}, []string{"authorization_code"}, []string{"openid"}, true, "active")
	assert.Error(t, updateErr)
	_, rotateErr := service.RotateClientSecret(t.Context(), testID("tenant"), testID("missing"))
	assert.Error(t, rotateErr)
	assert.Error(t, service.DeleteClient(t.Context(), testID("tenant"), testID("missing")))
}
...
func newOAuthTestService(t *testing.T) (*Service, *authnmod.WebService, string) {
	...
	require.NoError(t, err)
	principalID, err := authnmod.EnsureBootstrapPrincipal(context.Background(), db, authnmod.BootstrapPrincipal{LoginName: "admin", Password: "correct horse battery staple"}, oauthTestNextID)
	require.NoError(t, err)
 ...
 VALUES (?, ?, ?, ?, 'active', ?, ?, 0)`, testID("tenant"), principalID, "Admin", "", time.Now().UTC().UnixMilli(), time.Now().UTC().UnixMilli())
	require.NoError(t, err)
	return NewService(db, auth, oauthTestNextID), auth, guidString(principalID)
}

var oauthTestIDCounter atomic.Int64

func oauthTestNextID() (guid.ID, error) {
	return guid.ID(oauthTestIDCounter.Add(1)), nil
}
