...
func newProvisioningEnvironmentWithDB(t *testing.T, db *bun.DB, key []byte) provisioningEnvironment {
	...
	directory, err := service.CreateDirectory(t.Context(), testID("tenant-a"), "Primary IdP")
	require.NoError(t, err)
	credential, err := service.CreateCredential(t.Context(), testID("tenant-a"), directory.ID, "acceptance", nil)
	require.NoError(t, err)
...
var secureTestIDCounter atomic.Int64

func secureTestID() (guid.ID, error) {
	return guid.ID(secureTestIDCounter.Add(1)), nil
}
...
func TestProvisionedResourceLifecycle(t *testing.T) {
	...
	group, err = env.service.PatchGroup(t.Context(), env.auth, parseGUID(group.ID), PatchRequest{Schemas: []string{PatchSchema}, Operations: []PatchOperation{{Op: "remove", Path: `members[value eq "` + second.ID + `"]`}}}, 1)
	require.NoError(t, err)
	require.Len(t, group.Members, 1)
	assert.Equal(t, weakETag(2), group.Meta.Version)

	user, err = env.service.PatchUser(t.Context(), env.auth, parseGUID(user.ID), PatchRequest{Schemas: []string{PatchSchema}, Operations: []PatchOperation{{Op: "replace", Path: "displayName", Value: []byte(`"Alice Updated"`)}}}, 1)
	require.NoError(t, err)
	assert.Equal(t, "Alice Updated", user.DisplayName)
	_, err = env.service.ReplaceUser(t.Context(), env.auth, parseGUID(user.ID), activeUserInput("ext-alice", "alice", "alice@example.test"), 1)
	assert.ErrorIs(t, err, ErrResourceVersion)

	require.NoError(t, env.service.DeleteUser(t.Context(), env.auth, parseGUID(user.ID), 2))
	_, err = env.service.GetUser(t.Context(), env.auth, parseGUID(user.ID))
	assert.ErrorIs(t, err, ErrResourceMissing)
	var memberStatus string
	require.NoError(t, env.db.NewSelect().Table("iam_tenant_members").Column("status").Where("tenant_id = ? AND principal_id = ?", env.auth.TenantID, parseGUID(user.ID)).Scan(t.Context(), &memberStatus))
	...
	require.NoError(t, env.service.DeleteGroup(t.Context(), env.auth, parseGUID(group.ID), 2))
	_, err = env.service.GetGroup(t.Context(), env.auth, parseGUID(group.ID))
	assert.ErrorIs(t, err, ErrResourceMissing)
...
func TestDirectoryCredentialLifecycle(t *testing.T) {
	...
	_, err = env.service.ListDirectories(t.Context(), 0)
	assert.ErrorIs(t, err, ErrInvalidDirectory)
	_, err = env.service.CreateDirectory(t.Context(), testID("missing-tenant"), "Missing")
	assert.ErrorIs(t, err, ErrInvalidDirectory)
	credentials, err := env.service.ListCredentials(t.Context(), env.auth.TenantID, env.auth.DirectoryID)
	require.NoError(t, err)
	require.Len(t, credentials, 1)
	_, err = env.service.ListCredentials(t.Context(), testID("other-tenant"), env.auth.DirectoryID)
	...
	assert.ErrorIs(t, err, ErrInvalidDirectory)
	_, err = env.service.Authenticate(t.Context(), "Bearer "+env.auth.CredentialID.String()+".wrong-secret")
	...
	assert.ErrorIs(t, err, ErrUnauthorized)
	assert.ErrorIs(t, env.service.RevokeCredential(t.Context(), env.auth.TenantID, env.auth.DirectoryID, testID("missing")), ErrCredentialMissing)
...
func TestResourceInputAndAuthorizationFailures(t *testing.T) {
	...
	_, err = env.service.CreateGroup(t.Context(), env.auth, GroupInput{Schemas: []string{GroupSchema}, DisplayName: "Invalid", Members: []SCIMGroupMember{{Value: wireID("missing")}}})
