...
func TestTargetManagementHTTPWorkflow(t *testing.T) {
	...
	response := adminRequest(t, server.Client(), http.MethodPost, server.URL+"/iam/scim/targets", "tenant-a", createBody, nil)
	...
	response = adminRequest(t, server.Client(), http.MethodGet, server.URL+"/iam/scim/targets", "tenant-a", nil, nil)
	...
	response = adminRequest(t, server.Client(), http.MethodPut, server.URL+"/iam/scim/targets/"+issued.Target.ID, "tenant-a", stale, nil)
	...
	response = adminRequest(t, server.Client(), http.MethodPut, server.URL+"/iam/scim/targets/"+issued.Target.ID, "tenant-a", replaceBody, nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	decodeHTTPBody(t, response, &envelope)
	var replaced Target
	...
	pushTarget, err := env.service.CreateTarget(t.Context(), "tenant-a", "Okta API", providerServer.URL, "api-token")
	...
	response = adminRequest(t, server.Client(), http.MethodPost, server.URL+"/iam/scim/targets/"+pushTarget.Target.ID+"/push", "tenant-a", pushBody, nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	decodeHTTPBody(t, response, &envelope)
	...
	deadURL := dead.URL
	dead.Close()
	deadTarget, err := env.service.CreateTarget(t.Context(), "tenant-a", "Dead", deadURL, "dead-token")
	require.NoError(t, err)
	response = adminRequest(t, server.Client(), http.MethodPost, server.URL+"/iam/scim/targets/"+deadTarget.Target.ID+"/push", "tenant-a", pushBody, map[string]string{"Accept-Language": "ms-MY"})
	...
	response = adminRequest(t, server.Client(), http.MethodPost, server.URL+"/iam/scim/targets/"+pushTarget.Target.ID+"/deprovision", "tenant-a", pushBody, nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	assert.Equal(t, http.MethodDelete, provider.last(t).Method)

	// Invalid payload is a 422.
	response = adminRequest(t, server.Client(), http.MethodPost, server.URL+"/iam/scim/targets/"+pushTarget.Target.ID+"/push", "tenant-a", []byte(`{"resource_type":"Widget","resource_id":"x"}`), nil)
	assert.Equal(t, http.StatusUnprocessableEntity, response.StatusCode)

	// Delete the target and confirm it is gone.
	response = adminRequest(t, server.Client(), http.MethodDelete, server.URL+"/iam/scim/targets/"+issued.Target.ID, "tenant-a", nil, nil)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	response = adminRequest(t, server.Client(), http.MethodGet, server.URL+"/iam/scim/targets", "tenant-a", nil, nil)
	decodeHTTPBody(t, response, &envelope)
