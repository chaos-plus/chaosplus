...
func TestOrganizationModuleLifecycleAndRouteDeclarations(t *testing.T) {
	...
	credentials, err := authnmod.NewWebService(authnext.Config{Enabled: true, Issuer: "https://iam.example", SigningKey: key, MFA: authnext.MFAConfig{EncryptionKey: key}, Web: authnext.WebConfig{Enabled: true}}, db, authnmod.WithIDGenerator(newTestIDGenerator()))
	require.NoError(t, err)
	principalService := identity.NewService(db, realOrganizationAuditAppender(db), iam.NewAdministratorGuard(), newTestIDGenerator())
