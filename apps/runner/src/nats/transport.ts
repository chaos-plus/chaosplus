...
/**
 * NATS transport for the runner ↔ Go server-ai link (PRD §17.1/C3, F.1).
 * Subject scheme:
...
/**
 * Transport contract shared by the NATS and WS runner links. serve.ts only
...
export class NatsDaemonTransport {
  ...
  /** Connect and subscribe to the runner's command subject. */
  ...
  /** Announce this runner to the server-ai (request/reply). Non-fatal if absent. */
  async register(meta: Record<string, string>): Promise<void> {
    ...
    try {
      ...
      // server-ai not up yet — registration retried by reconnect later
    }
  }
