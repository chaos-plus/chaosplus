...
/**
 * Runner entry (PRD §5.3.1): connect to the server-ai over one authenticated
...
async function onCommand(
  ...
  switch (cmd.type) {
    case "spawn": {
      ...
      // Forward message/tool output as spawn-event so the server-ai can
...
async function main(): Promise<void> {
  ...
  console.log(`[runner] detected runtimes: ${runtimes.join(", ")}`);
  console.log(`[runner] ${NAME} connected to ${SERVER}, awaiting commands`);
