...
/**
 * WS runner transport (PRD §5.3.1): the runner holds one authenticated
 * WebSocket to the server-ai (`/api/machines/ws?token=..&name=..`), receives
 * commands, replies request/reply style, and pushes events + heartbeats.
 *
 * Protocol (JSON frames):
 *   control→runner: {type:"cmd", reqId, cmd:<RunnerCommand>}
 *   runner→control: {type:"reply", reqId, ok, data?}
 *   runner→control: {type:"event", event:<RunnerEvent>}
 *   control→runner: {type:"ping"}  →  {type:"pong"}
 */
