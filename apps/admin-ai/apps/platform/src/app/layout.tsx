...
export default function PlatformLayout() {
  ...
  const secondary = useMemo(() => SECONDARY[top] ?? [], [top]);
  ...
  useEffect(() => {
    ...
    document.title = [
      sub ? t(`nav.${sub.key}`) : undefined,
      topLabel,
      "chaos.plus",
    ]
      .filter(Boolean)
      .join(" · ");
  }, [top, secondary, location.pathname, t]);
  ...
  useEffect(() => {
    const sync = () => {
      try {
        ...
        const p = raw
          ? (JSON.parse(raw) as { nickname?: string; email?: string })
          : null;
  ...
  return (
    <div className="flex h-svh flex-col bg-background">
      <header className="sticky top-0 z-40 border-b bg-background shadow-sm">
        <div className="flex h-16 items-center gap-2 px-3 sm:h-14 sm:gap-3 sm:px-4">
          <div
            className="flex min-w-0 items-center gap-2 font-semibold"
            aria-label="chaos.plus"
          >
          ...
          <div className="ml-auto flex min-w-0 items-center gap-1 sm:gap-2">
            ...
            <SimpleSelect
              ...
              placeholder="选择实体…"
              triggerClassName="h-11 w-[min(10rem,35vw)] sm:h-9 sm:max-w-40"
            />
            {/* 配置管理 */}
            <DropdownMenu>
              <DropdownMenuTrigger
                aria-label="配置管理"
                className="hidden size-11 place-items-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:grid sm:size-9"
              >
              ...
              <DropdownMenuContent align="end" className="w-44">
                ...
                <DropdownMenuItem onClick={() => navigate("/team/machines")}>
                  接入配置
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger
                aria-label="用户菜单"
                className="flex min-h-11 min-w-11 items-center justify-center gap-2 rounded-full p-1 transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:min-h-9 sm:min-w-0 sm:justify-start sm:pr-2"
              >
                <Avatar className="size-9 sm:size-8">
                  <AvatarFallback className="bg-gradient-to-br from-primary to-accent text-xs text-primary-foreground">
                    {(session?.preferred_username ?? session?.subject ?? "U")
                      .slice(0, 1)
                      .toUpperCase()}
                  </AvatarFallback>
                </Avatar>
      ...
      <Dialog open={newEntityOpen} onOpenChange={setNewEntityOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="grid gap-1.5 py-2">
            <label htmlFor="new-entity-name" className="text-sm font-medium">
              名称
            </label>
            <Input
              id="new-entity-name"
          ...
          <DialogFooter>
            ...
            <Button
              className="cursor-pointer"
              onClick={() => void createEntityInline()}
