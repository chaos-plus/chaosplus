...
| 10 结构约束 | 跨平台门禁扫描 | 当前 owner 为 `check_gates.py --scope all --full`，统一执行 Go race/精确覆盖率、staticcheck、golangci-lint、govulncheck、前端真实测试/构建、文档链接/Mermaid/受保护构建及外部 IAM/PDP 依赖扫描；20 个 Python 脚本/policy 自测已通过，但 2026-08-14 Full gate 精确覆盖率 `apps/server` 为 `15140/17519`（`86.420458%`）、`apps/server-ai` 为 `1396/6056`（`23.051519%`），低于 90% 发布阈值，因此本项保持阻塞，不能沿用历史 PowerShell 全绿结论 |
