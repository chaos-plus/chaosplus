...
CREATE TABLE workspace_requirements (
 ...
 parent_id BIGINT NULL CHECK(parent_id IS NULL OR parent_id>0), title TEXT NOT NULL CHECK(length(title) BETWEEN 1 AND 300), description TEXT NOT NULL DEFAULT '', acceptance_criteria TEXT NOT NULL DEFAULT '',
 status TEXT NOT NULL CHECK(status IN ('draft','approved','rejected','closed')),
 ...
 FOREIGN KEY(parent_id) REFERENCES workspace_requirements(id) ON DELETE RESTRICT
);
CREATE INDEX idx_workspace_requirements_scope ON workspace_requirements(tenant_id,entity_id,deleted_at,status,updated_at);
CREATE INDEX idx_workspace_requirements_parent ON workspace_requirements(tenant_id,entity_id,parent_id,deleted_at);
-- +goose Down
