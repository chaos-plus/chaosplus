...
CREATE TABLE workspace_test_cases (
 ...
 requirement_id BIGINT NULL CHECK(requirement_id IS NULL OR requirement_id>0), title VARCHAR(300) NOT NULL CHECK(char_length(title) BETWEEN 1 AND 300), description TEXT NOT NULL DEFAULT '', preconditions TEXT NOT NULL DEFAULT '',
...
CREATE TABLE workspace_test_steps (
 ...
 action TEXT NOT NULL CHECK(char_length(action)>0), expected_result TEXT NOT NULL CHECK(char_length(expected_result)>0), created_at BIGINT NOT NULL CHECK(created_at>0), created_by BIGINT NOT NULL CHECK(created_by>0), updated_at BIGINT NOT NULL CHECK(updated_at>0), updated_by BIGINT NOT NULL CHECK(updated_by>0),
 deleted_at BIGINT NOT NULL DEFAULT 0 CHECK(deleted_at>=0), deleted_by BIGINT NOT NULL DEFAULT 0 CHECK(deleted_by>=0), version BIGINT NOT NULL DEFAULT 1 CHECK(version>=1),
 CHECK((deleted_at=0 AND deleted_by=0) OR (deleted_at>0 AND deleted_by>0)), CONSTRAINT fk_workspace_test_step_case FOREIGN KEY(test_case_id) REFERENCES workspace_test_cases(id) ON DELETE RESTRICT
);
