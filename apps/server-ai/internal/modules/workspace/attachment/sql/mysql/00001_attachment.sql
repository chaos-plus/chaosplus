...
CREATE TABLE workspace_attachments (
 ...
 resource_type VARCHAR(16) NOT NULL CHECK(resource_type IN ('requirement','task','objective','conversation')), resource_id BIGINT NOT NULL CHECK(resource_id > 0), filename VARCHAR(255) NOT NULL CHECK(char_length(filename) BETWEEN 1 AND 255), content_type VARCHAR(127) NOT NULL,
 size_bytes BIGINT NOT NULL CHECK(size_bytes BETWEEN 1 AND 104857600), checksum CHAR(64) NOT NULL CHECK(checksum REGEXP '^[0-9a-f]{64}$'), object_key VARCHAR(512) NOT NULL UNIQUE,
 ...
 INDEX idx_workspace_attachments_resource(tenant_id, entity_id, resource_type, resource_id, deleted_at, created_at), INDEX idx_workspace_attachments_deletion(status, deleted_at, updated_at)
