...
CREATE TABLE workspace_test_runs (
 ...
 status VARCHAR(16) NOT NULL CHECK(status IN ('queued','running','passed','failed','blocked','cancelled')), started_at BIGINT NOT NULL DEFAULT 0 CHECK(started_at>=0), completed_at BIGINT NOT NULL DEFAULT 0 CHECK(completed_at>=0), observed_result TEXT NOT NULL DEFAULT '', failure_summary TEXT NOT NULL DEFAULT '',
