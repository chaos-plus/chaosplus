...
CREATE TABLE workspace_objectives (
 ...
 title TEXT NOT NULL CHECK(length(title) BETWEEN 1 AND 300), description TEXT NOT NULL DEFAULT '', period_start BIGINT NOT NULL CHECK(period_start > 0), period_end BIGINT NOT NULL CHECK(period_end >= period_start),
...
CREATE TABLE workspace_key_results (
 ...
 title TEXT NOT NULL CHECK(length(title) BETWEEN 1 AND 300), target_value NUMERIC NOT NULL CHECK(target_value >= 0 AND target_value < 100000000000000), current_value NUMERIC NOT NULL CHECK(current_value >= 0 AND current_value < 100000000000000), unit TEXT NOT NULL CHECK(length(unit) <= 32),
 created_at BIGINT NOT NULL CHECK(created_at > 0), created_by BIGINT NOT NULL CHECK(created_by > 0), updated_at BIGINT NOT NULL CHECK(updated_at > 0), updated_by BIGINT NOT NULL CHECK(updated_by > 0),
 deleted_at BIGINT NOT NULL DEFAULT 0 CHECK(deleted_at >= 0), deleted_by BIGINT NOT NULL DEFAULT 0 CHECK(deleted_by >= 0), version BIGINT NOT NULL DEFAULT 1 CHECK(version >= 1),
 CHECK((deleted_at = 0 AND deleted_by = 0) OR (deleted_at > 0 AND deleted_by > 0)), FOREIGN KEY(objective_id) REFERENCES workspace_objectives(id) ON DELETE RESTRICT
);
