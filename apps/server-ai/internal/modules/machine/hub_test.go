...
func TestHubHandshakeInvalidToken(t *testing.T) {
	nc := startTestNATS(t)
	hub := NewHub(nc, NewTokenStore(), nil, testNextID(), secure.SameOriginPolicy())
...
// server-ai gateway → NATS chaos.runner.{id}.cmd → hub → WS cmd → daemon
...
func TestHubBridgeSpawn(t *testing.T) {
	...
	hub := NewHub(nc, tokens, nil, testNextID(), secure.SameOriginPolicy())
	ts := httptest.NewServer(http.HandlerFunc(hub.HandleWS))
	defer ts.Close()
	...
	if len(g.RegisteredRunners()) != 1 || g.RegisteredRunners()[0] != machineID.String() {
		t.Fatalf("gateway registered = %v, want [%s]", g.RegisteredRunners(), machineID)
	}

	// gateway spawns; daemon replies + sends spawn-done over WS.
	done := make(chan error, 1)
	go func() {
		res, err := g.SpawnAndWaitOpts(ctx, machineID.String(), gateway.Spawn{SpawnID: "s1", RunID: "r", NodeID: "n", ExecutorType: "mock", Prompt: "hi"}, gateway.WithMaxTimeout(5*time.Second))
		if err != nil {
			done <- err
			return
		}
