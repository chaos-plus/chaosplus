...
CREATE TABLE conversation_channels (
 ...
 name TEXT NOT NULL CHECK(length(name) BETWEEN 1 AND 120), topic TEXT NOT NULL DEFAULT '' CHECK(length(topic) <= 1000), status TEXT NOT NULL CHECK(status IN ('active','archived')),
...
CREATE TABLE conversation_channel_members (
 ...
 UNIQUE(tenant_id, entity_id, channel_id, member_id, kind), CHECK((removed_at = 0 AND removed_by = 0) OR (removed_at > 0 AND removed_by > 0)), FOREIGN KEY(channel_id) REFERENCES conversation_channels(id) ON DELETE RESTRICT
);
