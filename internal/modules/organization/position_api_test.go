...
func TestPositionHTTPWorkflow(t *testing.T) {
	...
	assert.Equal(t, http.StatusOK, api.Get("/iam/positions/"+position.ID, tenant).Code)
	...
	updated := api.Patch("/iam/positions/"+position.ID, tenant, map[string]any{"name": "Platform Engineer", "status": "disabled", "version": position.Version})
	require.Equal(t, http.StatusOK, updated.Code, updated.Body.String())
	position = decodePosition(t, updated.Body.Bytes())
	assert.Equal(t, int64(2), position.Version)
	assert.Equal(t, http.StatusConflict, api.Patch("/iam/positions/"+position.ID, tenant, map[string]any{"name": "Stale", "version": 1}).Code)

	start := time.Date(2026, 8, 2, 12, 0, 0, 0, time.UTC)
	end := start.Add(time.Hour)
	memberResponse := api.Put("/iam/positions/"+position.ID+"/members/principal%2Fa", tenant, map[string]any{"starts_at": start, "ends_at": end})
	require.Equal(t, http.StatusOK, memberResponse.Code, memberResponse.Body.String())
	members := api.Get("/iam/positions/"+position.ID+"/members", tenant)
	...
	assert.Equal(t, http.StatusConflict, api.Delete("/iam/positions/"+position.ID+"?version=2", tenant).Code)
	assert.Equal(t, http.StatusOK, api.Delete("/iam/positions/"+position.ID+"/members/principal%2Fa", tenant).Code)
	assert.Equal(t, http.StatusOK, api.Delete("/iam/positions/"+position.ID+"?version=2", tenant).Code)
	assert.Equal(t, http.StatusNotFound, api.Get("/iam/positions/"+position.ID, tenant).Code)
	assert.Equal(t, http.StatusNotFound, api.Get("/iam/positions/"+position.ID, authz.TenantHeader+": tenant-b").Code)
}
...
func TestPositionHTTPEmptyCollectionsAndErrors(t *testing.T) {
	...
	assert.Equal(t, http.StatusUnprocessableEntity, api.Delete("/iam/positions/missing?version=0", tenant).Code)
	assert.Equal(t, http.StatusNotFound, api.Put("/iam/positions/missing/members/missing", tenant, map[string]any{}).Code)
...
func TestPositionHTTPPreservesLastAdministrator(t *testing.T) {
	...
	require.Equal(t, http.StatusCreated, created.Code, created.Body.String())
	position := decodePosition(t, created.Body.Bytes())
	assigned := api.Put("/iam/positions/"+position.ID+"/members/administrator", tenant, map[string]any{})
	...
	for _, response := range []*httptest.ResponseRecorder{
		api.Patch("/iam/positions/"+position.ID, tenant, map[string]any{"status": "disabled", "version": position.Version}),
		api.Put("/iam/positions/"+position.ID+"/members/administrator", tenant, map[string]any{"ends_at": time.Now().UTC().Add(time.Hour)}),
		api.Delete("/iam/positions/"+position.ID+"/members/administrator", tenant),
	} {
