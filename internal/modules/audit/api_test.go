...
func TestAuditHTTPQueryAndIntegrity(t *testing.T) {
	...
	event, err := service.Append(t.Context(), EventInput{TenantID: "tenant", EventType: "client_created", TargetType: "oauth_client", TargetID: "client", Outcome: "success"})
	require.NoError(t, err)
	...
	assert.Equal(t, http.StatusOK, api.Get("/iam/audit-events/"+event.ID, header).Code)
	assert.Equal(t, http.StatusOK, api.Get("/iam/audit-integrity", header).Code)
	assert.Equal(t, http.StatusNotFound, api.Get("/iam/audit-events/missing", header).Code)
	...
	assert.Contains(t, exported.Header().Get("Content-Disposition"), "chaosplus-audit-")
...
func TestAuditExportUsesRealHTTPListener(t *testing.T) {
	...
	require.NoError(t, iam.Migrate(t.Context(), db))
	service := NewService(db)
	_, err = service.Append(t.Context(), EventInput{TenantID: "tenant", EventType: "client_created", Outcome: "success"})
	require.NoError(t, err)
	router := chi.NewMux()
	...
	assert.Contains(t, string(body), `"schema":"chaosplus.audit-export.v1"`)
...
func TestAuditHTTPFailsClosedWhenDatabaseUnavailable(t *testing.T) {
	...
	assert.Equal(t, http.StatusInternalServerError, api.Get("/iam/audit-events/event", header).Code)
