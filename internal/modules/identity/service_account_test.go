...
func TestServiceAccountHTTPAndOAuthLifecycle(t *testing.T) {
	...
	oauthService := oauth.NewService(db, authentication)
	...
	listed, total, err := identityService.ListServiceAccounts(t.Context(), "tenant", "report", 10, 0)
	require.NoError(t, err)
	require.Equal(t, int64(1), total)
	require.Len(t, listed, 1)
	assert.Equal(t, account.ID, listed[0].ID)
	empty, total, err := identityService.ListServiceAccounts(t.Context(), "tenant", "missing", 10, 0)
	require.NoError(t, err)
	assert.Zero(t, total)
	...
	credentialResponse := api.Post("/iam/service-accounts/"+account.ID+"/credentials", tenant, map[string]any{
	...
	require.True(t, strings.HasPrefix(credential.Credential.ID, "sac_"))
	require.NotEmpty(t, credential.Secret)

	listedCredentials := api.Get("/iam/service-accounts/"+account.ID+"/credentials", tenant)
	...
	token := exchangeServiceAccountToken(t, api, credential.Credential.ID, credential.Secret, "reports.read")
	...
	revoked := api.Delete("/iam/service-accounts/"+account.ID+"/credentials/"+credential.Credential.ID, tenant)
	...
	assert.Equal(t, http.StatusBadRequest, exchangeServiceAccountTokenResponse(api, credential.Credential.ID, credential.Secret, "reports.read").Code)

	secondResponse := api.Post("/iam/service-accounts/"+account.ID+"/credentials", tenant, map[string]any{
	...
	activeToken := exchangeServiceAccountToken(t, api, second.Credential.ID, second.Secret, "")

	disabled := api.Put("/iam/service-accounts/"+account.ID, tenant, map[string]any{
	...
	assert.ErrorIs(t, err, authnext.ErrInvalidToken)
	assert.Equal(t, http.StatusBadRequest, exchangeServiceAccountTokenResponse(api, second.Credential.ID, second.Secret, "").Code)

	restored := api.Put("/iam/service-accounts/"+account.ID, tenant, map[string]any{
	...
	account = accountEnvelope.Data
	activeToken = exchangeServiceAccountToken(t, api, second.Credential.ID, second.Secret, "reports.read")
	now := time.Now().UTC().UnixMilli()
	_, err = db.ExecContext(t.Context(), `INSERT INTO iam_roles (tenant_id,id,name,description,created_at,updated_at) VALUES ('tenant','administrator','Administrator','',?,?)`, now, now)
	require.NoError(t, err)
	_, err = db.ExecContext(t.Context(), `INSERT INTO iam_role_permissions (tenant_id,role_id,permission_code,created_at) VALUES ('tenant','administrator','tenant_administer',?)`, now)
	require.NoError(t, err)
	_, err = db.ExecContext(t.Context(), `INSERT INTO iam_role_members (tenant_id,role_id,user_subject,created_at) VALUES ('tenant','administrator',?,?)`, account.ID, now)
	require.NoError(t, err)
	lastAdministrator := api.Put("/iam/service-accounts/"+account.ID, tenant, map[string]any{
		"display_name": account.DisplayName, "description": account.Description, "status": "disabled", "version": account.Version,
	})
	...
	backup, err := identityService.Create(t.Context(), "tenant", "backup-admin", "backup administrator password", "Backup Admin", "")
	require.NoError(t, err)
	_, err = db.ExecContext(t.Context(), `INSERT INTO iam_role_members (tenant_id,role_id,user_subject,created_at) VALUES ('tenant','administrator',?,?)`, backup.ID, now)
	require.NoError(t, err)

	stale := api.Put("/iam/service-accounts/"+account.ID, tenant, map[string]any{
	...
	assert.Equal(t, http.StatusNotFound, api.Get("/iam/service-accounts/"+account.ID, authz.TenantHeader+": other").Code)

	deleted := api.Delete("/iam/service-accounts/"+account.ID+"?version="+fmtInt(account.Version), tenant)
	require.Equal(t, http.StatusOK, deleted.Code, deleted.Body.String())
	assert.Equal(t, http.StatusNotFound, api.Get("/iam/service-accounts/"+account.ID, tenant).Code)
	_, err = authentication.Authenticate(t.Context(), "Bearer "+activeToken.AccessToken, "")
	assert.ErrorIs(t, err, authnext.ErrInvalidToken)
	...
	auditEvents, err := db.NewSelect().Table("iam_audit_events").Where("target_id = ? OR detail LIKE ?", account.ID, "%"+account.ID+"%").Count(t.Context())
	require.NoError(t, err)
...
func TestServiceAccountTransactionsAndValidation(t *testing.T) {
	...
	_, err = service.CreateServiceAccount(t.Context(), "missing", "worker", "Worker", "", nil)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	_, err = service.CreateServiceAccount(t.Context(), "tenant", "", "", "", nil)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	past := time.Now().UTC().Add(-time.Minute)
	_, err = service.CreateServiceAccount(t.Context(), "tenant", "worker", "Worker", "", &past)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)

	rejectIdentityAuditEvent(t, db, "service_account_created")
	_, err = service.CreateServiceAccount(t.Context(), "tenant", "rollback", "Rollback", "", nil)
...
func TestServiceAccountCredentialLimitsAndRollbacks(t *testing.T) {
	...
	account, err := service.CreateServiceAccount(t.Context(), "tenant", "batch-worker", "Batch Worker", "", &accountExpiry)
	require.NoError(t, err)
	require.NotNil(t, account.ExpiresAt)

	_, _, err = service.ListServiceAccounts(t.Context(), "", "", 50, 0)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	_, err = service.GetServiceAccount(t.Context(), "tenant", "missing")
	assert.ErrorIs(t, err, ErrServiceAccountNotFound)
	_, err = service.ReplaceServiceAccount(t.Context(), "tenant", account.ID, "Batch Worker", "", "active", nil, 0)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	assert.ErrorIs(t, service.DeleteServiceAccount(t.Context(), "tenant", account.ID, 0), ErrServiceAccountInvalid)
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "", []string{"jobs.run"}, nil)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "invalid", nil, nil)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "invalid", []string{"jobs run"}, nil)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	past := time.Now().UTC().Add(-time.Minute)
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "expired", []string{"jobs.run"}, &past)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	beyondAccountExpiry := accountExpiry.Add(time.Hour)
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "too-long", []string{"jobs.run"}, &beyondAccountExpiry)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	assert.ErrorIs(t, service.RevokeServiceAccountCredential(t.Context(), "", account.ID, "credential"), ErrServiceAccountInvalid)

	credentialExpiry := time.Now().UTC().Add(time.Hour)
	rejectIdentityAuditEvent(t, db, "service_account_credential_created")
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "rollback", []string{"jobs.run"}, &credentialExpiry)
	require.ErrorContains(t, err, "append audit event")
	var count int
	...
	credential, err := service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "primary", []string{"jobs.write", "jobs.read", "jobs.read"}, &credentialExpiry)
	require.NoError(t, err)
	...
	err = service.RevokeServiceAccountCredential(t.Context(), "tenant", account.ID, credential.Credential.ID)
	require.ErrorContains(t, err, "append audit event")
	credentials, err := service.ListServiceAccountCredentials(t.Context(), "tenant", account.ID)
	require.NoError(t, err)
	...
	err = service.DeleteServiceAccount(t.Context(), "tenant", account.ID, account.Version)
	require.ErrorContains(t, err, "append audit event")
	current, err := service.GetServiceAccount(t.Context(), "tenant", account.ID)
	require.NoError(t, err)
	...
	assert.ErrorIs(t, service.RevokeServiceAccountCredential(t.Context(), "tenant", account.ID, "missing"), ErrCredentialNotFound)
	require.NoError(t, service.RevokeServiceAccountCredential(t.Context(), "tenant", account.ID, credential.Credential.ID))
	require.NoError(t, service.RevokeServiceAccountCredential(t.Context(), "tenant", account.ID, credential.Credential.ID))
	_, err = service.ListServiceAccountCredentials(t.Context(), "tenant", "missing")
	assert.ErrorIs(t, err, ErrServiceAccountNotFound)
	...
	require.NoError(t, err)
	assert.Error(t, service.DeleteServiceAccount(t.Context(), "tenant", account.ID, account.Version))
	...
	require.NoError(t, err)
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "rejected", []string{"jobs.run"}, nil)
	assert.Error(t, err)
	...
	assert.Equal(t, http.StatusOK, api.Get("/iam/service-accounts/"+account.ID, tenant).Code)
	assert.Equal(t, http.StatusNotFound, api.Delete("/iam/service-accounts/missing?version=1", tenant).Code)
	assert.Equal(t, http.StatusNotFound, api.Get("/iam/service-accounts/missing/credentials", tenant).Code)
	assert.ErrorIs(t, service.DeleteServiceAccount(t.Context(), "tenant", account.ID, account.Version+1), ErrServiceAccountConflict)
	_, err = service.GetServiceAccount(t.Context(), "", account.ID)
	assert.ErrorIs(t, err, ErrServiceAccountInvalid)
	...
	response = api.Delete("/iam/service-accounts/"+account.ID+"/credentials/missing", tenant)
	...
	response = api.Post("/iam/service-accounts/"+account.ID+"/credentials", tenant, map[string]any{"name": "overflow", "scopes": []string{"jobs.run"}})
	assert.Equal(t, http.StatusConflict, response.Code, response.Body.String())
	...
	_, err = service.GetServiceAccount(t.Context(), "tenant", account.ID)
	assert.Error(t, err)
	_, err = service.ReplaceServiceAccount(t.Context(), "tenant", account.ID, "Batch Worker", "", "active", &accountExpiry, account.Version)
	assert.Error(t, err)
	assert.Error(t, service.DeleteServiceAccount(t.Context(), "tenant", account.ID, account.Version))
	_, err = service.CreateServiceAccountCredential(t.Context(), "tenant", account.ID, "closed", []string{"jobs.run"}, nil)
	assert.Error(t, err)
	assert.Error(t, service.RevokeServiceAccountCredential(t.Context(), "tenant", account.ID, "closed"))
}
