...
func TestFederationMigrationLifecycle(t *testing.T) {
...
func TestFederationMigrationDialectLifecycle(t *testing.T) {
	...
	db := newFederationLifecycleDatabase(t, dialect, os.Getenv("IAM_DB_LIFECYCLE_ADMIN_DSN"))
	require.NoError(t, iam.Migrate(t.Context(), db))
	require.NoError(t, organization.Migrate(t.Context(), db))
	require.NoError(t, Migrate(t.Context(), db))
	require.NoError(t, AssertMigrated(t.Context(), db))
...
func newFederationLifecycleDatabase(t *testing.T, dialect, adminDSN string) *bun.DB {
	t.Helper()
	...
	name := fmt.Sprintf("chaosplus_federation_%d", time.Now().UTC().UnixNano())
	quoted := `"` + name + `"`
