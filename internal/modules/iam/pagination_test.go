...
func TestListEntitiesPagePushesPagingIntoTheDatabase(t *testing.T) {
	...
	require.NoError(t, organization.EnsureTenant(t.Context(), repo.db, "other"))
	for i := range 7 {
		_, err := repo.CreateEntity(t.Context(), iamdomain.Entity{
			TenantID: "tenant", Type: "store", Name: fmt.Sprintf("Store %02d", i), Status: iamdomain.EntityActive,
		})
		require.NoError(t, err)
	}
	_, err := repo.CreateEntity(t.Context(), iamdomain.Entity{
		TenantID: "other", Type: "store", Name: "Foreign", Status: iamdomain.EntityActive,
	})
	require.NoError(t, err)

	// The total reflects the whole tenant even when a page is requested.
	page, total, err := repo.ListEntitiesPage(t.Context(), "tenant", 0, 3)
	require.NoError(t, err)
	assert.Equal(t, int64(7), total, "total must not leak the other tenant")
	require.Len(t, page, 3)
	assert.Equal(t, "Store 00", page[0].Name)

	second, total, err := repo.ListEntitiesPage(t.Context(), "tenant", 3, 3)
	require.NoError(t, err)
	assert.Equal(t, int64(7), total)
	require.Len(t, second, 3)
	assert.Equal(t, "Store 03", second[0].Name)

	tail, _, err := repo.ListEntitiesPage(t.Context(), "tenant", 6, 3)
	require.NoError(t, err)
	assert.Len(t, tail, 1, "the final short page is not padded")

	past, _, err := repo.ListEntitiesPage(t.Context(), "tenant", 99, 3)
	require.NoError(t, err)
	assert.Empty(t, past)

	// A zero limit keeps the historical "everything" contract.
	all, total, err := repo.ListEntitiesPage(t.Context(), "tenant", 0, 0)
	require.NoError(t, err)
	assert.Equal(t, int64(7), total)
	assert.Len(t, all, 7)
}
...
func TestListRolesPagePushesPagingIntoTheDatabase(t *testing.T) {
	...
	require.NoError(t, organization.EnsureTenant(t.Context(), repo.db, "other"))
	for i := range 5 {
		_, err := repo.CreateRole(t.Context(), "tenant", fmt.Sprintf("Role %02d", i), "")
		require.NoError(t, err)
	}
	_, err := repo.CreateRole(t.Context(), "other", "Foreign", "")
	require.NoError(t, err)

	page, total, err := repo.ListRolesPage(t.Context(), "tenant", 1, 2)
	require.NoError(t, err)
	...
	all, total, err := repo.ListRolesPage(t.Context(), "tenant", 0, 0)
	require.NoError(t, err)
	assert.Equal(t, int64(5), total)
	assert.Len(t, all, 5)
}
...
func TestServicePagingValidatesTenant(t *testing.T) {
	...
	_, _, err := service.ListEntitiesPage(t.Context(), "", 0, 10)
	assert.ErrorIs(t, err, ErrInvalidArgument)
	_, _, err = service.ListRolesPage(t.Context(), "   ", 0, 10)
	assert.ErrorIs(t, err, ErrInvalidArgument)

	entities, total, err := service.ListEntitiesPage(t.Context(), "tenant", 0, 10)
	require.NoError(t, err)
	assert.Zero(t, total)
	assert.Empty(t, entities)

	roles, total, err := service.ListRolesPage(t.Context(), "tenant", 0, 10)
	require.NoError(t, err)
	assert.Zero(t, total)
