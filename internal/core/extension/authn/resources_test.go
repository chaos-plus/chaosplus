...
func TestRuntimeResourcesRoundTripAndResolve(t *testing.T) {
	...
	loginTokenPath := filepath.Join(t.TempDir(), "login-token")
	require.NoError(t, os.WriteFile(loginTokenPath, []byte("login-client-token\n"), 0o600))
	cfg, err := ResolveConfig(Config{ResourcesFile: path, Web: WebConfig{EncryptionKeyFile: keyPath, LoginClientTokenFile: loginTokenPath}})
	require.NoError(t, err)
	...
	assert.Equal(t, "login-client-token", cfg.Web.LoginClientToken)
	...
	_, err = ResolveConfig(Config{Web: WebConfig{LoginClientToken: "inline", LoginClientTokenFile: loginTokenPath}})
	assert.ErrorContains(t, err, "mutually exclusive")
