...
[架构文档](apps/docs/src/content/docs/architecture/iam-platform.md)。
访问申请、四眼审批与临时授权契约见
[访问治理](apps/docs/src/content/docs/iam/access-governance.md)。

Production bootstrap, Docker Compose quick start, TLS, and external-service
configuration are documented in [部署指南](apps/docs/src/content/docs/operations/deployment.md).
...
- **Self-hosted IAM** — local principals, Argon2id credentials, opaque browser
  ...
  [SCIM provisioning](apps/docs/src/content/docs/iam/scim-provisioning.md).
- **Signed WebAssembly claim plugins** — capability-limited WASM modules can enrich
  ...
  [plugin system](apps/docs/src/content/docs/architecture/plugin-system.md).
...
The repository gate is the authoritative cross-platform quality check:
...
parity (`en-US` / `zh-CN` / `ms-MY`), mock-free internal testing,
production-code miniredis exclusion, real-dependency release acceptance, test/production file pairing, and
frontend lint/typecheck/tests/build plus the documentation build and link
check. Required Go analysis tools are installed automatically at pinned versions.
